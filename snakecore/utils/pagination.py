...
class EmbedPaginator:
    ...
    def __init__(
        ...
        *pages: discord.Embed,
        ...
        self.whitelisted_role_ids = {role.id for role in whitelisted_roles} if whitelisted_roles is not None else None
    ...
    async def toggle_tutorial_page(self):
        ...
        if self.show_tutorial:
            embed_utils.edit_embed(self.paginator_info_embed, footer_text=f"Page {self.current_page_index+1} of {len(self.pages)}.")
    ...
    async def check_event(self, event: discord.RawReactionActionEvent):
        ...
        while not self.killed:
            try:
                event = await config.conf.global_client.wait_for(
                    "raw_reaction_add", timeout=60, check=(lambda event: event.message_id == self.message.id and isinstance(event.member, discord.Member) and not event.member.bot)
                )
